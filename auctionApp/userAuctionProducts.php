...
else 
{
    ...
    $products=$productBl->getProductByUserID($userID);
...
<body>
  ...
  <?php
    ...
    <div class='img-container'>
      <img src='{$products["data"][$i]["SLIKA"]}'>
    </div>
    <div class='product-info'>
      <div class='product-content'>
        <h1>{$products["data"][$i]["NAZIV"]}</h1>
        <p>{$products["data"][$i]["OPIS"]}</p>
        <div class='buttons'>
          <a class='button buy' href='singleProductPage.php?IDProduct={$products["data"][$i]["IDPROIZVOD"]}'>VIDI</a>
          <span class='button' id='price'>{$products["data"][$i]["POCETNACENA"]},00 RSD</span>
        </div>
      </div>
    </div>
  </div>
    ...
    {
        echo "Nemate novih proizvoda na aukciji!";
    }
    ?>
