...
<body>
  ...
  <?php
    ...
    if (isset($products["CATEGORY"] [4]))
    {
         
         
             for ($i=0; $i<count($products["CATEGORY"][4]["data"]);$i++)
             {
             echo "
             
                   <div class='product'>
    <div class='img-container'>
      <img src='{$products["CATEGORY"] [4]["data"][$i]["SLIKA"]}'>
    </div>
    <div class='product-info'>
      <div class='product-content'>
        <h1>{$products["CATEGORY"] [4]["data"][$i]["NAZIV"]}</h1>
        <p>{$products["CATEGORY"] [4]["data"][$i]["OPIS"]}</p>
        <div class='buttons'>
          <a class='button buy' href='singleProductPage.php?IDProduct={$products["CATEGORY"] [4]["data"][$i]["IDPROIZVOD"]}'>DODAJ PONUDU ZA OVAJ PROIZVOD</a>
          <br>
          <br>
          <span class='button' id='price'>{$products["CATEGORY"] [4]["data"][$i]["POCETNACENA"]},00 RSD</span>
        </div>
      </div>
    </div>
  </div>
